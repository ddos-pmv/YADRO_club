...
public:
    ArrivalEvent(int time, const std::string &detail, ComputerClub &club)
        :Event(time, detail, club){id = 1;};
    void execute() override;

};
class SeatEvent:public Event{
public:
    SeatEvent(int time, const std::string &detail, int tableId, ComputerClub &club)
            :Event(time, detail, club), tableId(tableId){id = 2;};
    void execute() override;
