...
bool EventFactory::validateTime(int hours, int minutes) {
    return (hours >= 0 && minutes >= 0 && hours < 25 && minutes < 61 &&
        ((minutes == 0 && hours == 24) || hours < 24));
}
...
EventPtr EventFactory::createEvent(std::string& line, ComputerClub& club) {
    ...
    if (!(iss >> hours >> delim >> minutes >> eventId) ||
        !validateTime(hours, minutes) &&
        club.getCurMaxTime() <= hours * 60 + minutes) {
        std::cout << line << '\n';
        return nullptr;
    }
...
void SeatEvent::execute() {
    ...
    if (client.currentTable != 0) {
        ...
        //save price at old table
        if (client.paidForTime < time) {
            oldTable.revenue +=
                club.getHourlyRate() * ((time - client.startTime + 59) / 60);
            client.paidForTime =
...
void LeaveEvent::execute() {
    ...
    if (client.currentTable != 0) {
        Table& oldTable = club.getTable(client.currentTable);
        oldTable.occupiedMinutes += time - client.startTime;
        //if price was conuted on prev table
        if (client.paidForTime < time) {
            oldTable.revenue +=
                club.getHourlyRate() * ((time - client.startTime + 59) / 60);
            client.paidForTime =
